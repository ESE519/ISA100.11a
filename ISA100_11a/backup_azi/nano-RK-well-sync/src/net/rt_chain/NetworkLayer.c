...
// From wd.c
extern int8_t wd_tx_packet_enqueue(uint8_t *buf, uint8_t len, uint16_t priority); 
...
inline int8_t add_neighbor(Neighbor n)
{
	...
	*/
	...
	} // end for initialise_network_layer
	...
	switch( nw_ctrl_type(pkt -> type) )
	{
		...
		case NGB_LIST:		// NGB_LIST msg
			...
			{					
				...
				//08/08/07 Bach: I temporarily comment out the following line, we dont need to send
				//the neighbor list to the gateway right now
...
void nl_rx_task()
{
	...
	if(val == NRK_ERROR)
	{
		...
		nrk_led_set(RED_LED);
		while(1)
			nrk_kprintf(PSTR("NL: Error returned by wd_init()\r\n"));
	}	
		
	// give the link layer a rx buffer 	
	val = wd_rx_pkt_set_buffer(rx_buf, RF_BUFFER_SIZE);
	if(val == NRK_ERROR)
	{
		nrk_int_disable();
		nrk_led_set(RED_LED);
		while(1)
			nrk_kprintf(PSTR("NL: Error returned by wd_rx_pkt_set_buffer()\r\n"));
	}
	...
	while(1)
	{
		...
		while(wd_rx_pkt_ready() == NRK_ERROR)
		{
			val = wd_wait_until_rx_packet();
			if(DEBUG_NL == 2)
			{
				nrk_kprintf(PSTR("NL: wd_wait_until_rx_packet() returned "));
				printf("%d\n", val);
			}
		}		
		// Get the packet 
	   	do
	   	{
	   		...
	   		{
	   			nrk_kprintf(PSTR("NL: NULL returned by wd_rx_pkt_get()\r\n"));
	   		}
		...
		if(flag == 1)
		{
			wd_rx_pkt_release();	// drop the packet and go receive another
...
void nl_tx_task()
{
	...
	// wait for the nl_rx_task to start wd 	
	while(!wd_started()) nrk_wait_until_next_period();
	
	// retrieve and register for the 'transmit done' signal from wd 	
	...
	// set the radio power
	if( wd_set_rf_power(10) == NRK_ERROR)
  	{
  		nrk_led_set(RED_LED);
  		nrk_int_disable();
  		while(1)
	...
	while(1)
	{
		...
		do
		{
			/*NOTICE: need to reconsider priority parameter here*/
			ret =  wd_tx_packet_enqueue(tx_buf, SIZE_NW_PACKET, PKT_PRIO);	// try to queue the buffer in link layer
			if(ret == NRK_ERROR)
		...
		nrk_led_set(BLUE_LED);
  		...
  		if(ret == 0)
  		{
	   	...
	   	if(ret & SIG(tx_done_signal)) 	// wd has successfully sent the packet over the radio 
		{
...
void print_RoutingTable(Msg_RoutingTable *mrtbl)
{
	...
	for(i = 0; i < MAX_NODES; i++)
	{
		//if((mrtbl -> rt[i]).dest == BCAST_ADDR)	// ignore invalid destination
		//	continue;
