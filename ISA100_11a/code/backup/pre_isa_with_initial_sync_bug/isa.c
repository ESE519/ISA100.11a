...
uint8_t EXPIRATION = 100;// each slot lasts for 10ms, so 100 slots last for 1s
uint8_t slot_expired;
...
void isa_set_channel_pattern(uint8_t pattern)
{
	...
	case 1:
	    ...
	    channelPattern[8] = 14;
	    channelPattern[9] = 21;
	    channelPattern[10] = 11;
	    channelPattern[11] = 15;
	    channelPattern[12] = 22;
	    channelPattern[13] = 17;
	    channelPattern[14] = 13;
	    channelPattern[15] = 26;
	    break;
	default:
	    break;
    }
...
//printf("%d\r\n",isa_rfTxInfo.pPayload[SRC_INDEX]);
	...
	if (n == 1) {// successfully received ACK
		...
		if(DHR & (1<<7)){
			...
			uint8_t offsetSec = time_correction/HIGH_TIMER_TICKES_PER_MILISEC+1;
			uint16_t offsetNanoSec = HIGH_TIMER_TICKES_PER_MILISEC-time_correction%HIGH_TIMER_TICKES_PER_MILISEC;
...
void isa_nw_task ()
{
	...
	if(_isa_sync_ok == 1){
		...
		currentChannel = channelPattern[(channelIndex)&0x0F];
