...
#include "../../app_pkt_handlers/xmpp_pkt.h"
...
void tx_task ()
{
  ...
  while (1) {
    ...
    p2p_pkt.subnet_mac[1] = 1;
    p2p_pkt.subnet_mac[2] = 1;
    ...
    sprintf( my_passwd, "default");
    xp.passwd=my_passwd;
    sprintf( dst_jid, "024A03F@sensor.andrew.cmu.edu");
    xp.jid=dst_jid;
...
//    ping_p2p_generate(&p2p_pkt);
	...
	unpack_peer_2_peer_packet(&p2p_pkt);
		...
		// Packet arrived and is good to go
              	...
              	printf( "subnet: %d ",p2p_pkt.subnet_mac);  
