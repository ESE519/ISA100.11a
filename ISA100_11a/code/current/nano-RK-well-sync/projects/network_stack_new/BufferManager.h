...
typedef struct ReceiveBufferUDP
{
	...
	int8_t prio;					// indicates the priority of the segment contained. Set by the sending node 
...
typedef struct TransmitBuffer
{
	...
	int8_t prio;					// priority of packet. This is set by sending task
...
int8_t get_tx_aq_size();
uint8_t get_avg_wait_time();
void collect_queue_stats(int8_t *, int8_t *, uint8_t *, uint8_t *);
uint32_t get_total_wait_time();
uint32_t get_total_pkts_ins();
int8_t port_to_socket_index(uint8_t port);


void record_tx_queue_full(NW_Packet *);
