...
void sendPacket(uint16_t destAddr, uint8_t length, uint8_t *payload, void (*slot_callback)(ISA_QUEUE *entry, status)) ;
...
bool isTransmitLinkPresent (uint16_t neighborAddr){
	uint8_t index;
	for (index = 0; index < DLMO_LINK_MAX_COUNT; index++)
			{
				if (dlmoLink[index].isPresent == true && dlmoLink[index].linkType == TX_NO_ADV && dlmoLink[index].neighbor->index==neighborAddr ) {
					return true;
				}
			}
	return false;
