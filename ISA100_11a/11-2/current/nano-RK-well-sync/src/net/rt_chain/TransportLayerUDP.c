...
int8_t send(int8_t sock_num, int8_t *ptr, int8_t len, int32_t dest_addr, int16_t dest_port, int8_t prio)
{
	...
	if(sock[sock_num].type == SOCK_RAW)
	{
		nrk_sig_t ll_tx_done_signal = wd_get_tx_signal();
		...
		if(sock[sock_num].timeout.secs != 0 || sock[sock_num].timeout.nano_secs != 0)
		{
  		...
  		*/
  		/*NOTE: need to recondsider piority here*/
  		while( wd_tx_packet(ptr, len, PKT_PRIO) == -1)
  		{
...
uint8_t* receive(int8_t sock_num, int8_t *len, uint16_t *srcAddr, uint8_t *srcPort, int8_t *rssi)
{
	...
	//nrk_kprintf(PSTR("hello 1..........\r\n"));
	if(sock[sock_num].timeout.secs == 0 && sock[sock_num].timeout.nano_secs == 0)
	{
		//nrk_kprintf(PSTR("hello 5..........\r\n"));
	...
	else // the user has specified a timeout value 
	{
		...
		//printf("hello %d %d %d.........\r\n",rbm_index, rx_buf_mgr[rbm_index].countFree,rx_buf_mgr[rbm_index].countTotal );
		//nrk_kprintf(PSTR("hello 6..........\r\n"));
		//if(rx_buf_mgr[rbm_index].countFree > 1) nrk_kprintf(PSTR("hello 8..........\r\n"));
		//if(rx_buf_mgr[rbm_index].countTotal < 0) nrk_kprintf(PSTR("hello 10..........\r\n"));
		//if(rbm_index == 0) nrk_kprintf(PSTR("hello 9..........\r\n"));
		if(rx_buf_mgr[rbm_index].countFree == rx_buf_mgr[rbm_index].countTotal)	// no queued segments 
		{
			if(DEBUG_TL == 2)
			{
			...
			{				
				...
				// check what signal we got 
   				...
   				else
   				{
   					nrk_int_disable();
					nrk_led_set(RED_LED);
					while(1)	nrk_kprintf(PSTR("receive(): Unknown signal received (with timeout)\r\n"));
				}
   			} // end while(1)
   		} // end if 
   	} // end else
   	...
   	if(buf == NULL)	// this should not happen 
   	{
   		nrk_int_disable();
		nrk_led_set(RED_LED);
		nrk_kprintf(PSTR("receive(): Bug found in implementation of data_arrived_signal / rx buffer mgmt. Exit! \r\n"));
		while(1);
