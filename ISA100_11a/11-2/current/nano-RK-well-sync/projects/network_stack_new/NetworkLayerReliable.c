...
#define MAJOR_CHANGE_THRESHOLD 10
...
extern void collect_queue_stats(int8_t *txqs, int8_t *rxqs, uint8_t *, uint8_t *);
extern uint32_t get_total_pkts_ins();
...
/************************************ DATA STRUCTURES *********************************/
uint16_t next_seq_no;									// each packet is numbered
SequenceNoCache snc;									// sequence number cache
...
uint8_t total_ack_time;									// statistics collection
uint16_t totalbytesSent;
uint16_t totalbytesReceived;
...
int8_t continue_sending_hello;						// flag to indicate whether to continue sending HELLO messages
int8_t hello_pkt_queued;							// to store whether the TX queue contains a HELLO message
	...
	*/
	...
	for(i = 0; i < MAX_NGBS; i++)	
	{
		...
		if(nl.ngbs[i].addr == n.addr)	// neighbor is found
		{
			...
			if(abs(nl.ngbs[i].rssi - n.rssi) >= MAJOR_CHANGE_THRESHOLD)		// large change in RSSI has occured
				continue_sending_ngblist = 1;	// ALLOWED here since Im already in the critical region
			nl.ngbs[i].rssi = n.rssi;			// assign the new RSSI
	...
	if(DEFAULT_GATEWAY != BCAST_ADDR)		// I know who the gateway is
		continue_sending_ngblist = 1;		// ALLOWED here since I am in the critical region
...
void update_NgbList()
{
	...
	uint16_t addr;	// temporary variable
	...
	for(i = 0; i < MAX_NGBS; i++)
		if(nl.ngbs[i].addr != INVALID_ADDR)		// valid entry in the Ngb_List array
		{
			...
			if(nl.ngbs[i].lastReport == 0)		// time to remove this neighbor
			{
				...
				// check if this neighbor is the 'nextHop' to the DG
				// ALLOWED here since already in the critical region
				for(j = 0; j < NUM_ROUTING_TABLE_ENTRIES; j++)
				{
					if(rt.rte[j].dest == DEFAULT_GATEWAY)	// this entry contains the route to DG
						if(rt.rte[j].nextHop == addr)		// removed neighbor is enroute to DG
						{
							// invalidate this entry in the routing table
							rt.rte[j].dest = INVALID_ADDR;
							rt.rte[j].nextHop = INVALID_ADDR;
							rt.rte[j].cost = (uint8_t)INFINITY;
							break;
						}
				} // end inner for
			} // end inner if
	...
	if(pkt -> nextHop != BCAST_ADDR)				// if nextHop == BCAST_ADDR, gateway should know about this
		pkt -> nextHop = route_addr(pkt -> dest);	// which neighbor should receive it?
	...
	else if(CONNECTED_TO_GATEWAY == TRUE) 			// i.e. the nextHop is BCAST_ADDR.  
		 {
			...
			memcpy(to_gw_buf_rx + SIZE_NODETOGATEWAYSERIAL_PACKET_HEADER, ntg_pkt.data, ntg_pkt.length);  
			sendToSerial(to_gw_buf_rx, SIZE_NODETOGATEWAYSERIAL_PACKET_HEADER + ntg_pkt.length);
		 } // end if
...
/**********************************************************************************************/
int8_t sendToGateway(uint8_t *ptr, int16_t len)	// user API // CHECKED
{
	...
	sendToSerial(gw_buf, SIZE_NODETOGATEWAYSERIAL_PACKET_HEADER + len);		// send the message to the gateway
...
void process_nw_ctrl_pkt(NW_Packet *pkt, int8_t rssi)
{
	...
	switch(pkt -> type)
	{
		case HELLO:		// HELLO msg
			...
			if(mh.dg == BCAST_ADDR)					// neighbor does not know address of DG as yet
			{
				if(DEFAULT_GATEWAY ==  BCAST_ADDR)	// I also do not know address of DG
					;								// do nothing
				else								// I know address of DG. Tell it to the neighbor
				{
					continue_sending_hello = 1;		// ALLOWED here since already in critical region
				}
			} // end if
			else 
			{
				if(DEFAULT_GATEWAY == BCAST_ADDR)	// neighbor knows address of DG, but I do not
					DEFAULT_GATEWAY = mh.dg;		// get the address of DG
				else if(DEFAULT_GATEWAY == mh.dg)  // both know DG and they are the same
				 			;						// do nothing
				 	 else							// both know DG, but they are different
				 	 {
				 	 	nrk_int_disable();
						nrk_led_set(RED_LED);
						printf("NL: process_nw_ctrl_pkt(): Two nodes in subnet have different DGs\r\n");
				 	 }
			} // end else
				...
				if(pkt -> nextHop == BCAST_ADDR)	// sender's routing tables need to be made
		 		{
					...
					//sendToSerial(to_gw_buf_rx, SIZE_NODETOGATEWAYSERIAL_PACKET); //for debugging
					memcpy(to_gw_buf_rx + SIZE_NODETOGATEWAYSERIAL_PACKET_HEADER, ntg_pkt.data, ntg_pkt.length);  
					sendToSerial(to_gw_buf_rx, SIZE_NODETOGATEWAYSERIAL_PACKET_HEADER + ntg_pkt.length); //for debugging
...
int8_t process_SeqNo(NW_Packet *pkt)
{
	...
	index = search_SeqNoCache(pkt -> src, &seqNo, &timestamp);
	if(index >= 0)								// found a match
	{
		nrk_time_get(&curr_time);				// get current time
		nrk_time_compact_nanos(&curr_time);
							
		if(pkt -> seqNo == seqNo)				// same sequence number
		{
			// check for freshness of entry
			if(curr_time.secs - timestamp >= SNC_TIMEOUT)	// entry is old
			{
				snc.snce[index].timestamp = curr_time.secs;	// update the timestamp
				return TRUE;								// this packet needs to be processed
			}
			else											// entry is new. Do not update timestamp
				return FALSE;								// this is a duplicate packet
		}
		else												// different sequence number
		{
			snc.snce[index].seqNo = pkt -> seqNo;			// update sequence number
			snc.snce[index].timestamp = curr_time.secs;		// update timestamp
			return TRUE;									// this packet needs to be processed
		}
	}
	// no match found
	add_to_SeqNoCache(pkt -> src, pkt -> seqNo);			// add this source node to the cache
	return TRUE;											// this packet needs to be processed
}								
/**********************************************************************************************/	
void nl_rx_task()	// CHECKED
{
	...
	while(1)
	{
		...
		// Get the packet 
	   	...
	   	// update statistics collection variable
	   	totalbytesReceived += len;
	   	
	   	if(len < SIZE_NW_PACKET_HEADER)	// bad packet
	   	{
	   		if(DEBUG_NL >= 0)
	   		{
		...
		memcpy(pkt_rx.data, local_rx_buf + SIZE_NW_PACKET_HEADER, pkt_rx.length);
		...
		{
			if(process_SeqNo(&pkt_rx) == FALSE)		// duplicate packet
				continue;
			
			if(DEBUG_NL >= 1)
			{
				nrk_kprintf(PSTR("NL: nl_rx_task(): Processing pkt with seqNo = "));
				printf("%u (%x) %u\r\n", pkt_rx.seqNo, pkt_rx.type, pkt_rx.src);
			}
...
void nl_tx_task()	// CHECKED
{
	...
	int8_t size_tx_buf;							// temporary variable
	 
	nrk_time_t time_elapsed, time_start, time_end;	// temporary variables
	int8_t num_retries;
	...
	while(1)	// begin processing forever
	{
		...
		/* The node needs to send HELLO messages under three cases
		Case 1: The TX queue is empty. 
		Case 2: a neighbor needs address of DG. I have address of DG and there is no HELLO message in TX queue
		Case 3: I need address of DG and there is no HELLO message in TX queue
		
		NOTE: continue_sending_hello is set only by nl_rx_task()
		      hello_pkt_queued is set only by nl_tx_task()
		*/		
		if( (get_tx_aq_size() == 0) || ((get_continue_sending_hello() == 1) && (hello_pkt_queued == 0)) || ((get_dg() == BCAST_ADDR) && (hello_pkt_queued == 0)) )	
		{
			...
			{	
				...
				{
					// only for debugging
					tx_aq_size = get_tx_aq_size();
					dfg = get_dg();
					nrk_kprintf(PSTR("Hello message time expired: "));
					printf("%lu (%d)(%u)\r\n", elapsed_hello.secs, tx_aq_size, dfg);
				}
				...
				{
					if(DEBUG_NL == 1)
						nrk_kprintf(PSTR("Unable to insert HELLO message into transmit queue\r\n"));
				}
				else
				{
					hello_pkt_queued = 1;
					set_continue_sending_hello(0);
				}
			...
			if(elapsed_ngblist.secs >= NGB_LIST_PERIOD)
		  	{
				...
				else 									// send the NGB_LIST message over the serial port
				{
					...
					memcpy(to_gw_buf_tx + SIZE_NODETOGATEWAYSERIAL_PACKET_HEADER, ntg_pkt.data, ntg_pkt.length);
					sendToSerial(to_gw_buf_tx, SIZE_NODETOGATEWAYSERIAL_PACKET_HEADER + ntg_pkt.length);
				} // end else
		...
		if(rand() % 10 >= 7)						// random number generator
		...
		if((ptr -> pkt).type == NGB_LIST)	// a NGB_LIST message is going to be transmitted
			...
			nodeinfo_pkt_dequeued = 1;	
			 else if((ptr -> pkt).type == HELLO)	// a HELLO message is going to be transmitted
			 	hello_pkt_queued = 0;				// NOTE: different way of handling
				
		// set the sequence number of this packet ONLY if generated by this node
		// else leave the sequence number the same
		if( (ptr -> pkt).src == NODE_ADDR )
			(ptr -> pkt).seqNo = next_seq_no++;		
		
		bmac_addr_decode_dest_mac(ptr -> pkt.nextHop);
		...
		memcpy(tx_buf + SIZE_NW_PACKET_HEADER, (ptr -> pkt).data, (ptr -> pkt).length);
		size_tx_buf = SIZE_NW_PACKET_HEADER + (ptr -> pkt).length;
		...
		num_retries = NUM_TX_RETRIES;
		do
		{
			nrk_time_get(&time_start);
			//ret = bmac_tx_pkt(tx_buf, SIZE_NW_PACKET);
			ret = bmac_tx_pkt(tx_buf, size_tx_buf);
			if(ret == NRK_OK)	
			{
				nrk_time_get(&time_end);	// statistics collection
				nrk_time_sub(&time_elapsed, time_end, time_start);
				nrk_time_compact_nanos(&time_elapsed);
				total_ack_time += (uint8_t)(time_elapsed.nano_secs/NANOS_PER_MS);
				totalbytesSent += size_tx_buf;
				break;					// successfully sent
			}
			else
			{
				num_retries--;			// number of retries for this packet
			}
		} while(num_retries > 0);
		
		if(num_retries == 0)			// check why we exited the while loop
		{
			nrk_kprintf(PSTR("BMAC did not successfully transmit this packet\r\n"));
		}
		else
		{
			// packet is sent. This packet will serve as an implicit HELLO message
			...
			if(isApplication == TRUE)		// it was an application layer packet 
			{
	           ...
	           leave_cr(tl_sem, 16);	
				 ...
				 else if(nodeinfo_pkt_dequeued == 1)
				 {
					...
					; 
		} // end else (packet was sent successfully
...
/*****************************************************************************************/  
  ...
  NL_TX_TASK.Ptos = (void *) &nl_tx_task_stack[511];
  ...
  NL_TX_TASK.cpu_reserve.secs = 4;
  NL_TX_TASK.cpu_reserve.nano_secs = 700 * NANOS_PER_MS;	
  NL_TX_TASK.period.secs = 5;
...
void initialise_network_layer()	// CHECKED
{
	...
	initialise_SeqNoCache();						// initialise the sequence number cache
	...
	hello_pkt_queued = 0;							// no HELLO messages are currently queued
	continue_sending_hello = 0;						// will be set by nl_rx_task; will be cleared by nl_tx_task
	
	// Zigbee specific initializations
	bmac_addr_decode_set_my_mac(NODE_ADDR);			// tell Zigbee my network address 
	bmac_addr_decode_enable();						// enable address decoding
	bmac_auto_ack_enable();							// enable AUTO-ACK
...
void initialise_routing_table()		// CHECKED
{
	...
	for(i = 0; i < NUM_ROUTING_TABLE_ENTRIES; i++)
	{
		...
		rt.rte[i].cost = (uint8_t)INFINITY;		  	  // no distances are known
	}
	return;
}
/*******************************************************************************************/
...
void initialise_SeqNoCache()
{
	...
	for(i = 0; i < NUM_ENTRIES_SNC; i++)
		snc.snce[i].addr = INVALID_ADDR;
...
int8_t search_SeqNoCache(uint16_t addr, uint16_t *seqNo, uint32_t *timestamp)
{
	...
	for(i = 0; i < NUM_ENTRIES_SNC; i++)
		if(snc.snce[i].addr == addr)	// found the match
		{
			*seqNo = snc.snce[i].seqNo;				// copy the sequence number and timestamp
			*timestamp = snc.snce[i].timestamp;
			return i;
		}
...
{
	int8_t i, min_time_index, empty_index;
	uint32_t min_time;
	nrk_time_t curr_time;
	
	empty_index = -1;
	min_time_index = -1;
	min_time = 0xFFFFFFFFUL;
		
	for(i = 0; i < NUM_ENTRIES_SNC; i++)
	{
		if(snc.snce[i].addr == INVALID_ADDR)	// empty slot
		{
			empty_index = i;
			break;
		}
		else if(snc.snce[i].timestamp <= min_time)	// find the oldest entry in this cache
			 {
			 	min_time = snc.snce[i].timestamp;
			 	min_time_index = i;
			 }
	}
	
	nrk_time_get(&curr_time);					// get the current time
	nrk_time_compact_nanos(&curr_time);
	if(empty_index != -1)						// calculate the index at which addition will take place
	{
		i = empty_index;
		snc.count++;
	}
	...
	snc.snce[i].addr = addr;
	snc.snce[i].seqNo = seqNo;
	snc.snce[i].timestamp = curr_time.secs;
...
/***********************************************************************************************/
void set_continue_sending_hello(int8_t value)
{
	enter_cr(nl_sem, 23);
	continue_sending_hello = value;
	leave_cr(nl_sem, 23);
...
void collect_node_statistics()
{
	...
	nrk_time_get(&curr_time);								// record timestamp
	nrk_time_compact_nanos(&curr_time);
	ns.timestamp = curr_time.secs;
	
	// record buffer stats
	collect_queue_stats(&(ns.tx_queue_size), &(ns.rx_queue_size), &(ns.total_pkts_inserted), &(ns.total_wait_time) );
	
	// record ACK time
	// no need of semaphores here since the variable is accessed only by one task
	ns.total_ack_time = total_ack_time;
	total_ack_time = 0;		// reset this variable
	
	// record bytes sent and received
	ns.bytesSent = totalbytesSent;
	ns.bytesReceived = totalbytesReceived;
	totalbytesSent = 0;		// reset the variables
	totalbytesReceived = 0;	
	
	// record active time of tasks
	fd = nrk_stats_get(0, &my_stats);	// get IDLE task statistics
	if(fd == NRK_ERROR)
	{
		nrk_kprintf(PSTR("NL: Error in collecting NRK statistics\r\n"));
		nrk_halt();
	}
	idle_time = _nrk_ticks_to_time(my_stats.total_ticks);
	nrk_time_sub(&active_time, curr_time, idle_time);
	nrk_time_compact_nanos(&active_time);
	ns.active_time = active_time.secs;	
