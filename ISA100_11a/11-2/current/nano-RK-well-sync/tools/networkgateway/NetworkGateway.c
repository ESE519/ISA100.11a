...
void disseminate_routing_tables()
{
	...
	for(row = 1; row <= top_mgr.count; row++)
	{
		...
		printf("Tx buffer before sending to Firefly: \r\n");
		printBuffer(tx_buf, SIZE_GATEWAYTONODESERIAL_PACKET);
...
int main()
{
	...
	*/
	...
	//sprintf (buffer, "This is a sample slip string: Counthfeuhfuefhuefheifheuifeifheufhfuhuef %d\n", cnt);

	//do{
	//ret = slipstream_send(tx_buf, 40);//(int)SIZE_GATEWAYTONODESERIAL_PACKET); // send an inital message to the server
